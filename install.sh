...
# Load nvm if available (most common Node.js install method)
export NVM_DIR="\$HOME/.nvm"
[ -s "\$NVM_DIR/nvm.sh" ] && source "\$NVM_DIR/nvm.sh" --no-use
[ -s "\$NVM_DIR/bash_completion" ] && source "\$NVM_DIR/bash_completion"

# Also check Homebrew and system paths
export PATH="\$NVM_DIR/versions/node/\$(ls \$NVM_DIR/versions/node 2>/dev/null | sort -V | tail -1)/bin:/opt/homebrew/bin:/usr/local/bin:\$PATH"
